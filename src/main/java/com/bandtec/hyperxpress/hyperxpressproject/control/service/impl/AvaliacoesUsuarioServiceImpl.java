...
public class AvaliacoesUsuarioServiceImpl implements AvaliacoesUsuarioService {
	...
	public List<AvaliacaoUsuarioDTO> pegarAvaliacoesPorUsuario(Long idUsuario) {
		List<AvaliacaoUsuarioDTO> avaliacoesUsuario = avaliacoesPorUsuario(idUsuario);
		if (avaliacoesUsuario.isEmpty()) {
			log.error("Avaliações para usuário de id {} nao encontradas.", idUsuario);
			throw new EmptyException("Avaliacao para Usuario");
		}
